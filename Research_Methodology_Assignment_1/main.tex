...
When developing promising AI, we must ensure that we select and further study the AI that shows potential by many factors, and not by a single fact. For instance, a specific AI may perform good, providing outputs with high accuracy, however, we must consider more factors, such as whether an AI can definitively explain its process in developing its conclusion, which is important for cases presented to the court; the idea must be logical, digestible and coherent. Theoretically, an AI can be excellent in detecting anomalies in a closed system, but may perform badly on real-world data, which makes practical experiments crucial when choosing an AI to further research on. Practical deployment must be chosen wisely, as some AI might be too costly computationally, consuming natural resources and the user-friendliness in this applications. 
...
The future of digital forensics should not strictly employ one methodology over another with absolute certainty. Instead, effecive cybercrime investigation requires the integration of standardized forensic practices, AI-assisted analysis, human expertise, and appropriate legal frameworks. 
% ---------------------------------------------------------------
