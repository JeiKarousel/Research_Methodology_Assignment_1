...
In the rapid advancement of technology and integration in the modern world, there is an higher dependence on digital devices and its software in our daily lives. As such, cybercrimes have become more prevalent, proving that increasingly sophisticated attacks render traditional manual forensic tools insufficient when digital evidence are in large datasets and encrypted. To understand its impacts and the current scene in digital forensics, this research article explores the role of digital forensics in cybercrime investigations through reviewing and analysis of 12 relevant studies. The analysed studies fall under 3 themes; standardised practices and advancements of digital forensics; role of AI in digital forensics; detection of digital deceptions. 

As a quality assurance, a general approach including a strict criteria were followed in selecting and analysing the research articles. Papers were sourced by trusted journals such as IEEE Xplore, ScienceDirect and IAES International Journal and were chosen by relevancy of its title, abstract and general content. The analysis was done by selective information extraction, scrutiny of its methodology, followed by a critical discussion leading to a final sysnthesis.

The findings indicate that while the current tools are still pertinent to cybercrime investigations in intelligence gathering, developing tools such as AI and machine learning demonstrate an efficient automation in detecting anomalies, classifying data, and analysis of images/videos. An advanced model, CNN-BiLSTM methods revealed to have a 94.8\% identification accuracy with efficient evidence extraction. 

However, the current studies highlights important research gaps that warrant further investigations, such as the lack of research methodology transparency, using poorly reflected real-world data in training models, and having few trusted studies on the emerging technological threats. 
This lack of global forensic protocols create cross-border digital forensic cases difficult, coupled with the lack of tool explainability which may lower defense and prosecution cases. 

Overall, the future of digital forensic research requires maintaining research transparency, with dependable datasets that better suit the current world under centralized global protocols. Developing tools such as AI models can be improved by providing real-time analysis, high accuracy detection of digital deceptions, whilst still being resourceful and not replace human judgement. Digital forensics must combine tools that are explainable, with established forensic procedures that must constantly adapt to advancing technologies,  maintaining integrity, with the emphasis on privacy. 
...
The methodological process of conducting the literature review was by following an ordered, systematic approach that ultimately results in synthesis of new information based on our chosen research articles. 

To begin, each article was thoroughly read to form an accurate context and understanding of the study conducted. Before extraction of any information, the aim was to fully comprehend the text, with any unfamiliar terms or procedures investigated to allow for an accurate depiction of the content. 

Subsequently, a second iteration of reading the text follows, with the main purpose to extract key findings, methodologies, and general points. The information was compiled into a comparative table in which summarized  each paper for easy reference. 

All methodologies per paper were scrutinized by analyzing how the study was conducted, the tools it used and the type of findings it obtained. The disadvantages and advantages were recorded and identified for further analysis.

Next, like articles in terms of area of study, general conclusions and method of research were grouped together, with an identification of a general theme and their limitations. For each theme, a critical analysis was executed.

After recognizing the groups, a draft was created to organize synthesized points to ensure the review follows a coherent order and is framed with an intuitive structure.
...
A combination of keywords were used as search terms such as “digital forensics,” “cybersecurity,” “investigation,” “artificial intelligence in digital forensics.” These words along with “AND” and “OR” boolean operators are used to refine the search and identify available and relevant papers. 
...
While analysing the research articles, there lies several trends 
One in particular being an Explainable AI (XAI), which are AI models that provide reasonings for their decisions alongside an interpretable output to support the legal admissibility of the detected digital deception alongside the general public's trust in their usage in digital forensics. \cite{SINGH2025103632}. 
Besides that, the facilitation of Federated Learning, which is a privacy-preserving approach where AI models can be trained across multiple devices without needing to centralise the raw biometric data \cite{SINGH2025103632}. 
In addition, there seems to be talks of implementing a continuous authentication: a system that monitor the user's general behaviors(gait, keystroke rhythm) throughout a whole session rather than relying on a one-time login \cite{SINGH2025103632}.
...
\subsection{The Practicality of Standardization}

In theme 1, the literature review focuses on the research articles that delves into topics such as standardized practice and advancements in digital forensics. This need for standardization stems from the problem that cybercrimes are becoming more elaborate and complex, as the technological scene is ever-advancing. Thus, digital forensics need a system whereby its evidence can be truly reliable and be accounted for legally.  

We have initially established that there are some standardized procedures implemented in digital forensic investigations. Standards enables evidence integrity, however, this raises an important question, how can the standard be referred to as a standard when each respective investigator has their own frameworks when conducting forensic investigations? The evidence obtained by each procedure will be inconsistent from each other and provide different insights, making it difficult to compare and analyze. Additionally, if 82.5\% of methodologies in studies are vague, how can we expect to draw scientific conclusions or conduct literature reviews when analyzing its content? In certain papers we have analyzed, most had used a simulation using generated data in a controlled environment, however, this does not reflect real-world data that varies in different situations. 

 In the event that the evidence is inaccurate, the critical analysis that follows would not hold any value nor substance, creating a challenging situation whereby if the research itself is unreliable, how will we depend on its results to pinpoint specific areas to research further and develop the necessary tools? 

Consequently, it is imperative to not continuously and blindly research areas without a defined foundation in the creation or collection of accurate datasets to the current world.  Once extensively researched, digital forensic models can then begin to be trained and be fed trusted data and provide proper evidence to its ability to be used as an effective tool, which may lead to further research. 

The tools that are developed may not be a full-proof solution, as while they are effective and efficient, if the person cannot interpret the data in a productive manner or explain its algorithm, the resulting evidence may not be permitted in court. 

This highlights the need for a constant education system for investigators that inform them of the emerging threats in cybersecurity, the tools to combat them, and how to proceed in their investigations. Standards in these investigations, whilst it is ideal to envision a global, universal protocol for all countries to abide by, it is more realistic to share certain cyber laws through association and collaboration, while each country maintains respective influence in their legal system.

\subsection{Implications of AI usage}

Theme 2 explores the literature on AI in digital forensics and its implications. In the various studies that we have dissected,  it is proven that AI is a helpful tool in evidence analysis and anomaly detection in large datasets, as technology is rapidly advancing to the point of humans not being able to manually handle. 

When developing promising AI, we must ensure that we select and further study the AI that shows potential by many factors, and not by a single fact. For instance, a specific AI may perform good, providing outputs with high accuracy, however, we must consider more factors, such as whether an AI can definitively explain its process in developing its conclusion, which is important for cases presented to the court; the idea must be logical, digestible and coherent. Theoretically, an AI can be excellent in detecting anomalies in a closed system, but may perform badly on real-world data, which makes practical experiments crucial when choosing an AI to further research on. Practical deployment must be done wisely, as some AI might be too costly computationally, consuming natural resources with the possible lack of user-friendliness in their applications. 

Furthermore, It is important to show transparency in how the AI works in their processing for evidence to be used in the court system, however, by exposing the method, it is a possibility that cyber criminals will be able to avoid such detections or bypass it, introducing a constant problem between public information rights and secrecy.

There must be a careful balance between AI usage and manual tools with human judgement, and ultimately, a person must be the one to interpret the data given by AI and to use it as evidence. 

AI, run by an algorithm, no matter how advanced, will not be capable of critically thinking with the minor nuances in specific contexts that can only be understood through human experiences. Furthermore, people can adapt immediately to a given situation, whilst AI must need new data to train on to make conclusions of any value. And in the event that the evidence presented needs to be scrutinized, AI cannot be held accountable for its findings, with the possibility of it hallucinating data, evidence and holding no conscience. Thus, it is crucial for AI to simply be an assistive tool rather than a replacement for human judgement.

\subsection{The Tug Between Deception \&  Detection}

Theme 3 concentrates on the analysis of deceptive techniques deployed by cybercriminals and its effects on their victims, demonstrating the need for further research on deepfake generations, email spoofing and social engineering. 

As established before, AI is a tool used to detect details in videos/images that may be deepfaked, which also consists of generation of data made by AI. It is known as a threat and a tool in digital forensics, with its role in deepfakes, steganography and encryption. Once a developer creates a new method of detecting AI, the attacker will adapt correspondingly, finding new methods to bypass the original system. This becomes an endless cycle of producing new forensic methods to detect ever-advancing deception techniques. 

Thus, creating a singular perfect model is not feasible, realistic nor should it be the focus of research. Different situations require different solutions, and what may work exceptionally well in one case may differ from the next. The goal in researching digital forensics should not be obtaining the perfect forensic system, we must instead create a system that constantly adapts to new situations, with its own ability to create sensible decisions in real-time and standard procedures that combine traditional forensic techniques. 

The future of digital forensics should not strictly employ one methodology over another with absolute certainty. Instead, effective cybercrime investigation requires the integration of standardized forensic practices, AI-assisted analysis, human expertise, and appropriate legal frameworks. 
% ---------------------------------------------------------------
...
Review methodologies for deepfake generation and detection & 
Literature Review \& Comparative Evaluation & 
Major public benchmarks datasets from media modalities & 
Detection Accuracy and Speed Trade-off, Requirements of governance and provenance integration&
Scalability \& Inference Bottlenecks & 
Lack of Explainable AI, Lack of suitable benchmark datasets 
\tabularnewline
\hline
...
Analyse the core domains of digital forensics and their limitations&
Systematic Literature Review & 
Peer Reviewed Literature published between 2019-2025 & 
Lack of Standardisation, high level theoretical metamodel & 
Resoruce Dependencies, Lack of Harmonised global legal standards & 
Lack of Unified Cross-domain framework
\tabularnewline
\hline
J. Osamor et al &
Comprehensive Forensic Analysis of a Spoofed Microsoft Email: A NIST Incident Response Approach & 
Forensic investigation through NIST Incident Response Lifecycle & 
Forensic inspection pipeline of metadata & 
Real-world email phishing sample from free online phishing sample repositories & 
Timestamp obfuscation, authentication failures and IP \& Domain discrepancy & 
Single sample case study & 
Integration of NIST framework, hybrid detection paradigms   
\tabularnewline
\hline
...
\textbf{Student:} Terry Teoh Jay Poh / 251UC2518F
\begin{table}[H]
\caption{Individual article review}
\label{tab:individual-article-review}
\vspace{-0.2cm}
...
K M Zubair et al. (2025) & 
Traditional forensic methods cannot keep up with the growing volume and complexity of digital evidence. & 
To test how AI can enhance digital forensic investigations through a hybrid deep learning model. & 
Quantitative experiment using a CNN-BiLSTM model with attention, compared against Logistic Regression, Random Forest, and LSTM. & 
Synthetic forensic data created in Python simulating phishing, insider theft, ransomware, DDoS, and log tampering. & 
The CNN-BiLSTM with attention performed best in accuracy, precision, recall, and F1-score, with the fewest false alarms and fastest evidence discovery. Removing any model component lowered performance. & 
Used only synthetic data, not real case files; small-scale simulation; no live deployment testing. & 
Need to test the model on real forensic data, reduce bias across regions and devices, and integrate it into real SOC and legal workflows.
\tabularnewline
\hline

Prajwalasimha S N et al. (2025) & 
Advanced Persistent Threats (APTs) are highly stealthy and can bypass traditional signature-based defenses. & 
To review AI-based solutions for detecting, attributing, and responding to APTs, and identify future research directions. & 
Literature review and case study analysis covering AI techniques such as deep learning, federated learning, and graph neural networks. & 
Real APT case studies including Operation Aurora, Stuxnet, Shady RAT, APT1, Sony Pictures Hack, and CloudSorcerer. & 
AI tools detect stealthy APTs better and produce fewer false alarms. A layered defense works best. Attribution remains hard due to custom malware and fileless techniques. & 
Adversarial ML attacks, imbalanced data, and high compute needs remain issues; attribution is mostly manual and unreliable. & 
Need for explainable AI, automated attribution, quantum-resistant cryptography, blockchain logging, and lightweight edge-compatible models.
\tabularnewline
\hline

Sheetal Tambe \& Pankaj Chandre (2026) & 
Data leakage is a major risk in hybrid environments, but current DLP systems are fragmented and not scalable. & 
To design and test a unified, privacy-preserving data leakage defense model for hybrid cloud, IoT, and enterprise systems. & 
Quantitative experiment testing the Entropy-Weighted Federated Data Leakage Model (EF-DLM) against rule-based DLP, ML-based DLP, and FedAvg. & 
Combined datasets: CICIDS 2023, Enron Email Corpus, and synthetic IoT sensor streams. & 
The proposed model achieved the highest accuracy and F1-score, with the fewest false positives and lower latency than most baselines. Entropy values stabilized over training. & 
Edge devices with weak hardware may struggle with real-time work; federated rounds add communication overhead; decisions are hard to explain. & 
Need explainability modules for audits, testing on live SaaS and IIoT systems, reducing edge-compute load, and proving cross-domain interoperability.
\tabularnewline
\hline
...
3 & [Digital forensics: Legal standards and practices in cybercrime investigation] & Wan Wei Siang &
Quantitative content analysis& 
40 reports from a quasi-experiment& 
General lack of validation and analytical quality in digital forensics reports & 
...
4 & [Comprehensive Forensic Analysis of a Spoofed Microsoft Email: A NIST Incident Response Approach] & 
Elsa Zara Binti Fakhurrazi &
Qualitative case study using the NIST Incident Response Lifecycle, with AbuseIPDB, MxToolbox, and VirusTotal. &
A suspicious Microsoft-themed phishing email from a free phishing sample site. &
Email was confirmed spoofed due to suspicious IP, failed SPF, DKIM and DMARC checks, mismatched sender domains, anonymous authentication, a Gmail reply-to, a tracking pixel, and a timestamp anomaly. &
Shows how manual forensic analysis using the NIST framework can uncover subtle phishing indicators often missed by automated tools. &
Only one email sample was used; no automated countermeasures proposed; manual analysis may not scale to high-volume SOC settings. &
Need for automated detection of subtle phishing indicators and integration of manual techniques into scalable SOC workflows.
\tabularnewline
\hline

5 & [Unveiling cybersecurity mysteries: A comprehensive survey on digital forensics trends, threats, and solutions in network security] & 
Elsa Zara Binti Fakhurrazi &
Systematic Literature Review across five major databases, covering six core digital forensics subdomains. &
Peer-reviewed journals, conference papers, and book chapters published between 2019 and 2025. &
Found high ambiguity and overlapping processes across subdomains, and proposed a unified metamodel. Noted that the presentation phase is missing in most domains and stressed the need for global standardization. &
Provides a cross-domain metamodel and lists actionable future directions such as quantum-resistant decryption, AI traffic analysis, federated learning, and blockchain. &
Theoretical with limited empirical validation; broad scope reduces depth; no primary data or experimental testing of the proposed model. &
Need for empirical validation of the metamodel and development of standardized forensic protocols across subdomains.
\tabularnewline
\hline

6 & [Unmasking digital deceptions: An integrative review of deepfake detection, multimedia forensics, and cybersecurity challenges] & 
Elsa Zara Binti Fakhurrazi &
Integrative review covering deepfake generation, detection techniques, and biometric defenses, with implementation-level detail. &
Publicly available datasets such as FFHQ, CelebA, VoxCeleb, FaceForensics++, DFDC, WildDeepfake, and ASVspoof. &
Mapped deepfake generation and detection methods, documented real-world attacks such as the CEO voice fraud and the Ali Bongo incident, and proposed explainable AI, federated learning, and policy frameworks as solutions. &
Combines technical detection methods with policy and ethical frameworks, and covers cross-dataset generalization and real-world forensic failures. &
Detection models risk becoming outdated as generative methods evolve; dataset biases limit generalizability; real-time edge detection is still impractical due to high compute cost. &
Need for inclusive standardized datasets, multimodal cross-domain detection, adversarial robustness, global regulatory harmonization, and explainable AI.
\tabularnewline
\hline

7 & [AI-Augmented Digital Forensics: Enhancing Cybersecurity Investigations through Intelligent Evidence Analysis] & 
Terry Teoh Jay Poh &
Tested a hybrid CNN-BiLSTM model with attention against Logistic Regression, Random Forest, and a standard LSTM model. &
Synthetic forensic data created in Python, simulating phishing, insider theft, ransomware, DDoS, and log tampering. &
The CNN-BiLSTM with attention performed best across all metrics, produced the fewest false alarms, and found key evidence much faster than the other models. Removing any part of the model lowered its performance. &
Shows that AI can speed up evidence discovery while keeping false alarms low, with attention heatmaps that help explain the model's decisions. &
Used only synthetic data, not real case files; small-scale simulation; no live deployment testing was done. &
Future work should test the model on real forensic data, reduce bias across regions and devices, and integrate it into real SOC and legal workflows.
\tabularnewline
\hline

8 & [AI-Powered Defense Against Advanced Persistent Threats (APTs): Techniques, Case Studies, and Future Research Directions] & 
Terry Teoh Jay Poh &
Reviewed existing research and analyzed real APT case studies covering AI detection, attribution, deception, and response methods. &
Case studies include Operation Aurora, Stuxnet, Operation Shady RAT, APT1, Sony Pictures Hack, NKAbuse, Triangulation Spyware, and CloudSorcerer. &
AI tools detect stealthy APTs better and produce fewer false alarms. A layered defense combining Zero Trust, IDPS, EDR/XDR, UEBA, and SIEM/SOAR works best. Attribution is still hard because attackers use custom malware and fileless techniques. &
Gives a full picture of the APT lifecycle, real-world attacks, and current defense frameworks, and points out clear future directions. &
Adversarial ML attacks, imbalanced data, and high compute needs remain issues; attribution is mostly manual and unreliable; real-time deployment is limited. &
Research is needed on explainable AI, automated attribution, quantum-resistant cryptography, blockchain logging, and lightweight edge-compatible models.
\tabularnewline
\hline

9 & [Bridging the Gaps in Data Leakage Defense: A Comprehensive Performance Evaluation of Detection and Prevention Frameworks] & 
Terry Teoh Jay Poh &
Tested the Entropy-Weighted Federated Data Leakage Model against rule-based DLP, ML-based DLP, and FedAvg federated DLP. &
Combined three datasets: CICIDS 2023, Enron Email Corpus, and synthetic IoT sensor streams. &
The proposed model achieved the highest accuracy and F1-score while producing the fewest false positives and lower latency than most baselines. Entropy values stabilized over training, showing growing model confidence. &
Entropy-weighted aggregation handles mixed data sources well, keeps data private, and scales linearly, unlike encryption or blockchain-based methods. &
Edge devices with weak hardware may struggle with real-time work; federated rounds add communication overhead; decisions are hard to explain; testing was limited to benchmark datasets. &
Need explainability modules for audits, testing on live SaaS and IIoT systems, reducing edge-compute load, and proving cross-domain interoperability.
\tabularnewline
\hline
